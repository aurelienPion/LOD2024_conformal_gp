...
def friedman(x):
    ...
    d = 5
    """
    return 10**gnp.sin(gnp.pi*x[:, 0]*x[:, 1]) + 20*(x[:, 2] - 0.5)**2 + 10*x[:, 3] + 5*x[:, 4]
...
def wrapper_becker(u, V, W, d, alpha, beta, gamma):
    ...
    def decorate(func):
        ...
        def wrap(x):
            ...
            sum_two = 0
            for i in range(d_2):
                sum_two += beta[i]*func_base[u[V[i, 0]]](
                    x[:, V[i, 0]])*func_base[u[V[i, 1]]](x[:, V[i, 1]])

            sum_three = 0
            for i in range(d_3):
                sum_three += gamma[i]*func_base[u[W[i, 0]]](x[:, W[i, 0]])*func_base[u[W[i, 1]]](
                    x[:, W[i, 1]])*func_base[u[W[i, 2]]](x[:, W[i, 2]])
